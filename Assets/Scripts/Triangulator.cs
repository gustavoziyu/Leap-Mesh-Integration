...
public class Triangulator : MonoBehaviour
{
    ...
    public void createPolygon(List<GameObject> v, int size)
    {
        // Create Vector2 vertices
        Vector3[] vertices2D = new Vector3[size];
        Vector3 center = new Vector3(0, 0, 0);

        for (int i = 0; i < size; i++)
        {
            vertices2D[i] = v[i].transform.position;
            center += v[i].transform.position;
        }
